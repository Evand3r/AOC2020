let arr = ["2-8 t: pncmjxlvckfbtrjh",
"8-9 l: lzllllldsl",
...
arr.forEach(a => {
  let [rule, pass] = a.split(':');
  let [limit, char] = rule.split(' ');
  limit = limit.split('-');
  pass = pass.trim();
  
  //Part1
  let found = (pass.match(new RegExp(char, 'gi')) || []).length;
  valid1 += found >= limit[0] && found <= limit[1];
  
  //Part2
  let equalAt1 = pass[limit[0] - 1] == char;
  let equalAt2 = pass[limit[1] - 1] == char;
  valid2 += (equalAt1 && !equalAt2) || (!equalAt1 && equalAt2);
})
